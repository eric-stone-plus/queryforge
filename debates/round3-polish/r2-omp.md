...
**Reviewer:** OMP (kimi-v2.5-pro)
**Date:** 2026-07-04
**Inputs:** 5 R1 artifacts (CW, OC, KC, Kimi, OMP), task.md, 10 source files
...
| D | Kimi | Clean prioritization tables, risk-aware |
...
| Kimi (D) | Self-Correction + Multi-view (Dashboard) | 2h+ |
...
**C. Time allocation: conservative (KC) vs aggressive (OC/Kimi)**
...
- **Kimi** plans 18h of work across 4 phases
...
4. **No one counted the total LLM calls per demo.** If self-correction is added, one failed query = 2 LLM calls. With Kimi rate limits, running 5 demo queries (some failing) could hit throttling. **Action: verify Kimi rate limits before demo.**
...
| 6 | Self-correction loop in agent.ts | 2h | +3-4 (Innovation) | CW, OC, KC, Kimi |
...
**C. Kimi's "Metric Memory" (Option B) — unloved but interesting**

Kimi proposed injecting saved metrics into the system prompt so the agent "remembers" previous queries. None of the other auditors picked this up.
...
- **OC, Kimi, OMP:** Dashboard should be the **landing page** (first impression = dashboard)
...
- **KC, Kimi:** Don't prioritize it (time sink, marginal demo value)
...
- **Kimi:** Brief hook (20s), then intro, then demo
...
- **Kimi:** Position 4 of 5 (2:00-2:30) — late
...
- **Kimi:** Include it in 5-min flow only, not 3-min
...
4. **No one addressed the "pre-warm" timing.** CW suggests hitting `/api/chat` 30s before demo. But if Kimi cold-starts, the first call could take 15-30s. **Action: pre-warm 2-3 minutes before, not 30 seconds.**
...
| 6 | Pre-warm Kimi API 2-3 min before demo | 0h |
...
| Metric rerun `history.length === 0` guard is a demo-breaking bug | **4/5** (OC, KC, Kimi, OMP; CW lists it under SHOULD) |
...
- **OC, KC, Kimi, OMP:** MUST fix — demo-breaking
...
- **OC, KC, Kimi, OMP:** 30s is acceptable (not listed as MUST)
...
- **OC, KC, Kimi:** SHOULD or not mentioned
...
2. **No one mentioned the Kimi API key on Railway.** The key needs to be set as a Railway environment variable. If the `.env.local` issue (mentioned in PROJECT-MEMO) persists, the Railway deploy will fail silently. **Action: verify env var works in production mode before deploying.**
...
| 4 | Set Kimi API key as Railway env var | 5min |
...
3. **Deployment timing moved earlier.** OC and Kimi convinced me that deploying at hour 4 is better than hour 11. The pivot-time argument is decisive.
...
| Q2: Innovation | MEDIUM | Self-correction might not work reliably with Kimi |
