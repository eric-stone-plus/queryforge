...
txt(s, M, Inches(4.8), Inches(10), Inches(0.4), "面向跨境电商小团队的 local-first macOS 分析工作台", Pt(13), ACCENT)
...
bullets(s, [
    ...
    "常用问题沉淀为指标，不需要每次从零分析",
    "桌面 macOS app 是真实产品，数据连接与 API key 留在本机",
], Inches(2.5))
...
footer(s, "Railway / iPhone QR 是 polished public demo；完整产品是本地 macOS app")
...
# 7 数据边界
s = prs.slides.add_slide(prs.slide_layouts[6]); bg(s)
title(s, "数据边界设计 — 本地工具也要可信")
bullets(s, [
    "默认暴露 schema；只把回答当前问题所需的查询结果交给模型解释",
    ...
    "OpenAI-compatible endpoint、model、API key、token budget 和数据连接都留在使用者本机",
], Inches(2.0))
txt(s, M, Inches(5.2), Inches(10), Inches(0.5), "关键不是替代模型，而是把外部模型 provider 接入受控数据、指标和执行边界", Pt(13), ACCENT)
footer(s, "QueryForge 不给 LLM 数据库权限 · AI 生成 SQL，数据库执行查询，策略引擎控制访问")

# 8 商业化路径
s = prs.slides.add_slide(prs.slide_layouts[6]); bg(s)
title(s, "商业化路径：广交会场景下的买断制工具")
...
footer(s, "Olist 巴西电商数据用于公开 demo；真实价值是把小卖家已有经营数据变成可追问的分析工作台")
